(() => {
    ...
    class Hook {
        ...
        trackSelectionRange(isInEditor) {
            if (this.trackingSelectionRange) {
                return;
            }
            this.trackingSelectionRange = true;
...
<div class="fontify-facebook" aria-label="Change Style">𝓢</div>');
    ...
    class TwitterHook extends Hook {
        isApplicable() {
            return (location.hostname.endsWith('.twitter.com') || location.hostname === 'twitter.com');
        }
        ...
        initialize() {
            ...
            if (this.locationButton && this.locationButton.parent()) {
                this.toolbar = this.locationButton.parent();
            }
        }
        ...
        injectInterface() {
            ...
            this.toolbar.append('<div aria-disabled="true" aria-label="Fontify" role="button" tabindex="-1" class="fontify-twitter"><div>𝓢</div></div>\
<div aria-disabled="true" aria-label="Fontify" role="button" tabindex="-1">\
<select id="fontify-twitter-font-selector">' + this.fontsOptions() + '</select>\
</div>');
            ...
            fontifyTwitter.click(function() {
                ...
                if (selectionRange && !selectionRange.collapsed) {
                    ...
                    if (selectedFont !== '') {
                        ...
                        self.selectionRange = replaceContents(selectionRange, false);
                    }
